...
(defun 2048-draw-grid (grid message)
  ...
  (let ((inhibit-read-only t))
    (erase-buffer)
    (insert "\n\n\tMerge matching numbers with the arrow or vi keys to reach 2048.")
...
(defvar 2048-mode-map
  (let ((map (make-sparse-keymap '2048-mode-map)))
    ...
    (define-key map "h"		'2048-move-left-action)
    (define-key map "l"		'2048-move-right-action)
    (define-key map "k"		'2048-move-up-action)
    (define-key map "j"		'2048-move-down-action)
    map))
