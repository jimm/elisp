...
;;
;; Version 1.0 written by Luke Gorrie <luke@xxxxxxxx> in February 2006
  ...
  (let ((content (buffer-string)))
    (with-temp-buffer
      ...
      (let ((request (buffer-string)))
        ...
        (set-process-sentinel http-twiddle-process 
'http-twiddle-process-sentinel)
  ...
  (if (and http-twiddle-endpoint (null current-prefix-arg))
    ...
    (setq http-twiddle-endpoint
          (list (read-string "Host: (default localhost) "
                ...
                (let ((input (read-from-minibuffer "Port: " nil nil t 
'http-twiddle-port-history)))
                  (if (integerp input)
                      input
...
close\n\nThis is the POST body.\n")
  (message "Now press `C-c C-c' and enter a webserver address (e.g. google.com 
port 80)."))
